...
class SplitableDiscreteDistribution:
    ...
    def get_entropy(self):
        """
        Calculate current entropy of the distribution for logging
        """
        total = self.count.sum()
        if total == 0:
            return 0.0
        probs = self.count / total
        # Avoid log(0)
        probs = probs[probs > 0]
...
class DiscreteDistributionOutput(nn.Module):
    ...
    # === [C-DDN Hyperparameters Defaults] ===
    # These are class variables that can be overridden, but also stored in instance
    alpha_ortho = 0.0
    beta_weak = 0.0
    # ===============================================
    ...
    def forward(self, d):
        ...
        if self.training:  # train
            ...
            if "max_distance" in d:  # random sample for DivergeShaping
                ...
                # === [New DDN Loss Calculation] ===
                # Use instance variables
                loss_attract = loss_func(predicts, targets).mean()
                loss_ortho = torch.tensor(0.0, device=device)
                loss_weak = torch.tensor(0.0, device=device)

                # Mask for losers
                if self.alpha_ortho > 0 or self.beta_weak > 0:
                    mask = torch.ones((b, self.k), dtype=torch.bool, device=predicts.device)
                    mask[torch.arange(b), idx_k] = False
                
                # (3) Weak Attraction Loss (Stability)
                if self.beta_weak > 0:
                    targets_expanded = targets.unsqueeze(1)
                    dist_from_gt = ((outputs - targets_expanded) ** 2).mean(dim=(-3, -2, -1))
                    loss_weak = dist_from_gt[mask].mean()
                
                # (4) New Orthogonal Loss (Feature/Weight Level Diversity)
                if self.alpha_ortho > 0:
                    # 1x1 Conv 가중치 가져오기: [K * predict_c, in_c, 1, 1]
                    # 우리가 원하는 건 K개의 '전문가'가 서로 직교하는 것
                    
                    # 가중치를 [K, -1] 형태로 평탄화 (각 전문가를 하나의 벡터로 봄)
                    # self.multi_out_conv1x1.conv1x1.weight 에 접근 (AdaptConv2d 사용 시 주의)
                    if hasattr(self.multi_out_conv1x1, 'conv1x1'): # AdaptConv2d인 경우
                        weight = self.multi_out_conv1x1.conv1x1.weight
                    else: # 일반 Conv2d인 경우
                        weight = self.multi_out_conv1x1.weight
                    
                    # K개의 전문가 벡터로 변환
                    W_flat = weight.view(self.k, -1)
                    
                    # 코사인 유사도 기반 직교성 (Normalize -> Gram Matrix)
                    W_norm = torch.nn.functional.normalize(W_flat, p=2, dim=1)
                    gram_matrix = torch.mm(W_norm, W_norm.t()) # (K, K)
                    
                    # Identity Matrix와의 차이를 최소화 (대각선은 1, 나머지는 0)
                    I = torch.eye(self.k, device=device)
                    loss_ortho = ((gram_matrix - I) ** 2).mean()

                # Total Loss 합산
                total_loss = loss_attract + \
                            (self.alpha_ortho * loss_ortho) + \
                            (self.beta_weak * loss_weak)

                d["loss"] = total_loss

                # [Logging Storage]
                # Store individual components for W&B logging in mnist.py
                # Use list to handle multiple DDN layers (RNN steps)
                d.setdefault("log_attract", []).append(loss_attract.detach())
                d.setdefault("log_weak", []).append(loss_weak.detach())
                d.setdefault("log_ortho", []).append(loss_ortho.detach()) # [추가]
                # d.setdefault("log_total", []).append(total_loss.detach())
                
                # Store winner indices for Histogram by Layer
                d.setdefault("layer_idx_k", {})[self.idx] = idx_k.detach()

            d["losses"] = d.get("losses", []) + [d["loss"]] # For backward compatibility if needed
